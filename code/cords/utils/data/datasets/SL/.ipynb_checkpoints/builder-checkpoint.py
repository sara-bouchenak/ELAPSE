...
from cords.utils.data.datasets.SL.celeba_loader import MyImageDataset
...
def gen_dataset(datadir, dset_name, feature, seed=42, isnumpy=False, **kwargs):
    ...
    if dset_name == "dna":
        ...
        if feature == 'classimb':
            ...
            samples_per_class = torch.zeros(num_cls)
            for i in range(num_cls):
                samples_per_class[i] = len(torch.where(trainset.smile == i)[0])
            min_samples = int(torch.min(samples_per_class) * 0.1)
            selected_classes = rng.choice(np.arange(num_cls), size=int(kwargs['classimb_ratio'] * num_cls), replace=False)
            for i in range(num_cls):
                if i == 0:
                    if i in selected_classes:
                        subset_idxs = list(
                            rng.choice(torch.where(trainset.smile == i)[0].cpu().numpy(), size=min_samples,
                                             replace=False))
                    else:
                        subset_idxs = list(torch.where(trainset.smile == i)[0].cpu().numpy())
                else:
                    if i in selected_classes:
                        batch_subset_idxs = list(
                            rng.choice(torch.where(trainset.smile == i)[0].cpu().numpy(), size=min_samples,
                        ...
                        batch_subset_idxs = list(torch.where(trainset.smile == i)[0].cpu().numpy())
                    subset_idxs.extend(batch_subset_idxs)
