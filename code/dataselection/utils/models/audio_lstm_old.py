...
class AudioLSTM(nn.Module):
    ...
    def __init__(
        ...
        # >>> AJOUT: dimension d'embedding exposée pour la sélection
        self.embDim = self.hidden * (2 if self.bidirectional else 1)
    ...
    # >>> AJOUT: compatibilité API (last, freeze) tout en gardant l’usage identique
    def forward(self, x: torch.Tensor, last: bool = False, freeze: bool = False):
        """
        Args:
            x: Tensor des features, formes listées en docstring.
            last: si True, retourne (logits, embedding).
            freeze: si True, on fige l'extracteur (LSTM + pooling + dropout).

        Returns:
            logits: [B, num_classes]  (ou (logits, embedding) si last=True)
        """
        ...
        if freeze:
            with torch.no_grad():
                ...
                z = self._temporal_pool(h)    # [B, H*dir]
                e = self.drop(z)              # embedding (après dropout)
        else:
            h, _ = self.lstm(x)
            z = self._temporal_pool(h)
            e = self.drop(z)

        logits = self.fc(e)                    # [B, num_classes]
        return (logits, e) if last else logits

    # >>> AJOUT: méthode requise par les stratégies de sélection
