...
class CRAIGStrategy(DataSelectionStrategy):
    ...
    def compute_score(self, model_params, idxs):
        ...
        if self.if_convex:
            for batch_idx, (inputs, targets, sensitive_attributes) in enumerate(subset_loader):
    ...
    def get_similarity_kernel(self):
        ...
        """
        for batch_idx, (inputs, targets, sensitive_attributes) in enumerate(self.trainloader):
            if batch_idx == 0:
                labels = targets
            else:
